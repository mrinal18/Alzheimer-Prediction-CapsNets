...
# -*- coding: utf-8 -*-
"""
Created on Thu May 17 18:33:18 2018
...
def main(options):
    ...
    use_cuda = (options.gpuid >= 1)
    # if options.gpuid:
    #     cuda.set_device(options.gpuid[0])

    # Training process
    model = CapsuleNet(options.num_classes)
    if CUDA:
        model.cuda()
    else:
        model.cpu()
...
#    margin_loss = MarginLoss()
    ...
    max_acc = 0
    max_epoch = 0
    f1 = open("capsule__loss_train", 'a')
    f2 = open("capsule_loss_train", 'a')
    for epoch_i in range(options.epochs):
        ...
        for it, train_data in enumerate(train_loader):
            ...
            img_input = imgs#.unsqueeze(1)integer_encoded = labels.data.cpu().numpy()
        ...
        if max_acc >= 0.75:
            torch.save(model.state_dict(), open("3DCapasule_model_" + str(epoch_i) + '_' + str(max_acc), 'wb'))
